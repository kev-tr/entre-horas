...
@export_category("Dias")
@export var dias_disponiveis: Array[int] = []
...
@export var tem_prazo: bool = false
@export var hora_prazo: int = 23
...
func esta_disponivel(minutos_do_dia: float) -> bool:
	...
	var fim := hora_fim * 60
	if tipo_compromisso == TipoCompromisso.HORARIO_MARCADO:
		return (
			minutos_do_dia >= hora_inicio * 60
			and minutos_do_dia < hora_prazo * 60
		)
		
	if tem_prazo:
		return (
			minutos_do_dia >= inicio
			and minutos_do_dia < fim
			and minutos_do_dia < hora_prazo * 60
		)
...
func pode_ser_concluida(minutos_do_dia: float) -> bool:
	if tipo_compromisso == TipoCompromisso.HORARIO_MARCADO:
		return esta_disponivel(minutos_do_dia)
	...
	return (
		esta_disponivel(minutos_do_dia)
		and minutos_do_dia + duracao_minutos <= hora_fim * 60
	)
